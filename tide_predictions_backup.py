...
STATION_IDS = [
    ...
    "8656483", "8658120", "8658163", "8659084", "8661070", "8665530", "8670659",
    "8670870", "8679511", "8720197", "8720218", "8720587", "8721147", "8721604",
...
def fetch_station_constituents(station_id):
    ...
    service that has historically gone down."""
...
def ensure_constituents(cache):
    ...
    for station_id in STATION_IDS:
        ...
        if entry and entry.get("fetched_at"):
            ...
            needs_refresh = age > timedelta(days=REFRESH_DAYS)
...
def predict_station(entry, start_date, days):
    """Compute hi/lo tide predictions for one station over
    [start_date, start_date+days) using cached harmonic constituents.
    ...
    # Start the UTC search a day early so the full local (ET) calendar day
    # of start_date is covered even though ET midnight falls a few hours
    # after UTC midnight -- otherwise the first local day can come back
    # with only a partial set of extrema.
    t0 = datetime(start_date.year, start_date.month, start_date.day) - timedelta(days=1)
    t1 = t0 + timedelta(days=days + 1)
...
def main():
    ...
    for station_id in STATION_IDS:
        ...
        if not entry:
            ...
            continue
        try:
            backup["stations"][station_id] = predict_station(entry, start_date, PREDICT_DAYS)
        except Exception as e:
